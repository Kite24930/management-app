<x-self.template title="PM Daily Report" css="reports.css">
    <main class="w-full md:pl-80 pt-24 md:pt-4 pb-24 flex flex-col flex-wrap items-center gap-4 relative z-10 full">
        ...
        <h2 class="text-2xl font-bold text-slate">Report Edit</h2>
        @if($errors->any())
        ...
        <div class="w-full max-w-4xl p-6">
            <form method="POST" class="w-full flex flex-col gap-4">
                ...
                <div class="flex gap-2 items-center">
                    ...
                    <input type="date" name="date" id="date" class="p-2 border border-slate rounded-lg" value="{{ date('Y-m-d', strtotime($report->date)) }}" required>
                </div>
                <div class="flex gap-2 items-center">
                ...
                <div id="reportWrapper" class="flex flex-col gap-2">
                    ...
                    @foreach($report_tasks as $report_task)
                        <x-self.report-task :tasks="$tasks" :num="$loop->index" :target="$report_task"/>
                    @endforeach
                </div>
    ...
    <script>
        ...
        window.Laravel.report_tasks = @json($report_tasks);
        window.Laravel.csrfToken = "{{ csrf_token() }}";
        window.Laravel.taskAddUrl = "{{ route('reports.task.components') }}";
        window.Laravel.saveUrl = "{{ route('reports.edit', $report->id) }}";
        window.Laravel.redirectUrl = "{{ route('reports.view', $report->id) }}";
        console.log(window.Laravel);
    </script>
    @vite(['resources/js/reports/reports-edit.js'])
</x-self.template>
